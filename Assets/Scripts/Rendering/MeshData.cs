...
public static class ChunkMeshGeneratorThreaded
{
        ...
        Func<int, int, int, BlockStateContainer> getState, int lodScale, NeighborLODInfo neighbors)
    {
        ...
        BlockEval[] evalCache = new BlockEval[CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE];
        bool[] evalReady = new bool[evalCache.Length];
        ...
        {
            GreedyDirection(getBlock, getState, dir, mesh, mask, lodScale, neighbors, evalCache, evalReady);
        }

        AddCustomBlockMeshes(getBlock, getState, mesh, lodScale, evalCache, evalReady);
        return mesh;
    }
    ...
    private static void GreedyDirection(Func<int,int,int,byte> getBlock, Func<int, int, int, BlockStateContainer> getState,
        Vector3Int dir, MeshData mesh, MaskCell[,] mask, int lodScale, NeighborLODInfo neighbors, BlockEval[] evalCache, bool[] evalReady)
    {
        int neighborScale =
        ...
        for (int w = 0; w < CHUNK_SIZE; w += lodScale)
        {
            ...
            for (int u = 0; u < CHUNK_SIZE; u += lodScale)
            {
                ...
                for (int v = 0; v < CHUNK_SIZE; v += lodScale)
                {
                    ...
                    BlockEval currentEval = GetBlockEval(getBlock, getState, x, y, z, current, evalCache, evalReady);
                    ...
                    {
                        BlockEval neighborEval = GetBlockEval(getBlock, getState, nx, ny, nz, neighbor, evalCache, evalReady);
    ...
    private static BlockEval GetBlockEval(
        ...
        bool[] evalReady)
    {
        if ((uint)x >= CHUNK_SIZE || (uint)y >= CHUNK_SIZE || (uint)z >= CHUNK_SIZE || evalCache == null || evalReady == null)
            return BuildBlockEval(getBlock, getState, x, y, z, knownBlock);

        int index = ToCacheIndex(x, y, z);
        if (evalReady[index])
            return evalCache[index];

        BlockEval eval = BuildBlockEval(getBlock, getState, x, y, z, knownBlock);
        ...
        byte knownBlock)
    {
        ...
        byte blockId = knownBlock != 0 ? knownBlock : getBlock(x, y, z);
